...
if prog == "0":
    set_temp = input("Enter the constant setpoint temperature (F): ")
    ...
    set_temp_str = str(set_temp_celsius) #convert to str to send command to change sp 
        ...
        init_temp = input("Enter the starting temperature (F): ")
        init_temp = float(init_temp)
        init_temp_celsius = ((init_temp - 32) * 5 / 9 )
        init_temp_celsius = float(init_temp_celsius) # Convert to float

        target_temp = input("Enter the target/peak temperature (F): ")
        target_temp = float(target_temp)
        target_temp_celsius = ((target_temp - 32) * 5 / 9)
        target_temp_celsius = float(target_temp_celsius) # convert to float
        
        rise_rate = input("Enter temperature ramp rate (F per hour): ")
        rise_rate = float(rise_rate)
        rise_rate_celsius = ((rise_rate - 32) * 5 / 9 )
        rise_rate_celsius = float(rise_rate_celsius)

        hold_time = input("Enter time to hold at target temperature (min): ")
        hold_time = float(hold_time)

        fall_rate = input("Enter temperature fall rate (F per hour): ")
        fall_rate = float(fall_rate)
        fall_rate_celsius = ((fall_rate - 32) * 5 / 9 )
        fall_rate_celsius = float(fall_rate_celsius)

        end_temp = input("Enter the ending temperature (F): ")
        end_temp = float(end_temp)
        end_temp_celsius = ((end_temp - 32) * 5 / 9 )
        end_temp_celsius = float(end_temp_celsius)
elif prog == "2":
        target_temp = input("Enter the target/peak temperature (F): ")
        target_temp = float(target_temp)
        target_temp_celsius = ((target_temp - 32) * 5 / 9 )
...
if prog == "0":
        ...
        response=bath.readline()
        while flag != True:
            print ("Setting constant temperature: %2.2f F" % set_temp)
            # Assemble the command to send to the water bath
            command = ("SS " + "%2.2f\r" % set_temp_celsius).encode('utf-8')
            bath.write(command)
            response = bath.readline()
            ...
            if response == set_temp_celsius:
                print ("Setpoint set: %2.2f F" % response)
...
if prog == "1":
        ...
        response=bath.readline()
        while flag != True:
            print ("Setting initial temperature: %2.2f F" % init_temp)
            # Assemble the command to send to the water bath
            command = ("SS " + "%2.2f\r" % init_temp_celsius ).encode("utf-8")
            bath.write(command)
            ...
            print ("Init-temp = %2.2f F" % init_temp)
            print ("Response = %2.2f F" % response)
            if response - 0.5 <= init_temp <= init_temp + 0.5:
                print ("Setpoint set: %2.2f F" % response)
                flag = True  # set True to kill while loop
        ...
        while flag != True:
            ...
            response = bath.readline()
            response = float(response[0:5])
            print ("Current bath temp: %2.2f F" % response)
            # When the bath temperature gets within 0.05 of the target, we're 
            # close enough
            if (abs(init_temp_celsius - response) < 0.1):
                flag = True  # set True to kill while loop
...
if prog == "2":
    ...
    init_temp_celsius = response # Set init_temp 
    ...
    temp_diff = abs(target_temp_celsius - init_temp_celsius)

    # Calculate the time needed for the ramp (degrees / degrees per hour)
    ramp_duration = temp_diff / rise_rate_celsius # units hours
    ...
    rise_rate_m = rise_rate_celsius / 60
    ...
    # correctly.
    if (target_temp_celsius - init_temp_celsius) < 0:
    ...
    while flag != True:
        ...
        if new_time > (prev_time + 60):
            ...
            if current_set < target_temp_celsius and not decrease_flag: # this may be issue if C not passed
                command = ("SS %2.2f\r" % current_set).encode('utf-8')
                bath.write(command) # update water bath setpoint
                response = bath.readline()
                # Calculate remaining temperature to cover
                temp_left = target_temp_celsius - current_set # may be issue if C not passed
                ...
                if time.localtime().tm_min % 1 == 0:
                    ...
                    print ("Current setpoint: %2.2f F, finishing at approx. %s") % \
                        (current_set,final_str)
            elif current_set >= target_temp_celsius and not decrease_flag:
                ...
                # this while loop
                current_set = target_temp_celsius # set current_set to the final 
                                          # target_temp
                command = ("SS %2.2f\r" % current_set).encode('utf-8')
                bath.write(command)
    ...
    # Calculate the time needed for the ramp (degrees / degrees per hour)
    fall_duration = temp_diff / fall_rate_celsius # units hours
    ...
    fall_rate_m = fall_rate_celsius / 60
